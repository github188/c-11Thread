...
int CDeviceManger::AddDevice(MangerDeviceInfo deviceInfo)
{
	...
	{
		if ( strcmp((*iter).szIp , deviceInfo.szIp) == 0 &&(*iter).iPort == deviceInfo.iPort)
		{
			if (strcmp((*iter).szUserName, deviceInfo.szUserName) == 0 && strcmp((*iter).szPwd, deviceInfo.szPwd) == 0)
			{
...
int CDeviceManger::DeleteDevice(MangerDeviceInfo deviceInfo)
{
	...
	for (; iter != m_ManualDeviceInfo.end(); iter++)
	{
		...
		{
			LOG_INFO("Del Device: name : %s , ip : %s ,port : %d", deviceInfo.szName, deviceInfo.szIp, deviceInfo.iPort);
			m_ManualDeviceInfo.erase(iter);
			return 0;
		}
	}
...
{
	int nRet = 0;
	std::cout << "DeviceManger mainLoop" << std::endl;
...
{
	std::string url = "http://127.0.0.1:80/services/DeviceInfoService?wsdl";
	char szUrl[256] = { 0 };
